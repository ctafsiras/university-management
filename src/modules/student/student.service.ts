...
const updateSingleStudent = async (
  ...
  const { name, guardian, localGuardian, ...updatableData } = updatedData;
  const newData = updatableData;

  if (name && Object.keys(name).length) {
    Object.keys(name).forEach((key) => {
      const newKey = `name.${key}` as keyof Partial<IStudent>;
      (newData as any)[newKey] = name[key as keyof typeof name];
    });
  }
